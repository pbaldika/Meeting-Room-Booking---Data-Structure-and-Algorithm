...
void searchTillHour(node * &hed, node* &til)
{
	...
	while(temp != NULL)
	{
		if (na==temp->data->getName() &&temp->data->getInstitution()==inst && temp->data->pesan->getYearfr() == test.getYear() && temp->data->pesan->getMonthfr() == test.getMonth() && temp->data->pesan->getDayfr() == test.getDay() && temp->data->pesan->getHourfr()==test.getHour())
  		{ 
			temp->data->print();
